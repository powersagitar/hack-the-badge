...
//! badge reads 8 of its 9 buttons through that chip rather than via 9 direct
//! GPIO pins (the 9th, `START`, is a direct GPIO line).
//!
...
//!
//! Standard parallel-in/serial-out shift register behavior. The shift register
//! carries **8 real button slots**, addressed by index (`0..8` of
//! [`Hc165::set_button`]); `PIN_START` is separate, a direct GPIO9 line
//! handled by [`Gpio::set_start_pressed`] rather than by this chip.
//!
//! **Bit order for shift-out**: the bit presented on `HC165_DATA` immediately
//! after a `LOAD` pulse (before any `CLK` edges) is slot 0; each subsequent
//! `CLK` rising edge advances to the next slot. So one load-plus-7-clocks
//! cycle presents `[slot0, slot1, …, slot7]`.
//!
//! **Which slot is which button** was left unresolved by Task 4 (it had no
//! working display to confirm against) and is now settled, from the same
//! third-party source the pin map above came from — a community member's own
//! working ESP-IDF firmware for this exact badge model
//! (`github.com/abigail-liang/hack-the-north-2026-badge`, `firmware/main.c`),
//! verified by its author against real hardware. Its `buttons()` routine
//! pulses `LOAD`, then reads `HC165_DATA` and clocks eight times, packing
//! read `i` into bit `i` of a mask it indexes with
//! `enum { BTN_A, BTN_B, BTN_HOME, BTN_DOWN, BTN_LEFT, BTN_RIGHT, BTN_UP,
//! BTN_AUX1 }`. Read `i` is exactly this model's slot `i`, so:
//!
//! | slot | button |
//! |------|--------|
//! | 0    | A      |
...
//! firmware shows it carries `AUX1`, a maintained side switch, so a hardcoded
//! constant would have made that button permanently unpressable. It still
//! *defaults* to released, so the earlier tests written against the
//! constant-bit behavior hold unchanged.
//!
...
//! latches a single snapshot of all 8 button slots at
...
//! saturates at the last (8th) bit — this model does not simulate a serial-in
//! pin or a daisy-chained second chip, so clocking past the 8th bit simply
//! keeps re-presenting that same last bit rather than shifting in undefined
//! data.
//!
//! **Reset/pre-`LOAD` state**: before any `LOAD` pulse has ever happened,
//! the latched snapshot defaults to "all released" (every slot high) and the
//! shift position starts at index 0 —
...
/// Number of real button slots the 74HC165 carries — see the module doc's
/// slot→button table.
pub const HC165_SLOTS: usize = 8;
...
pub struct Hc165 {
    ...
    /// the frozen snapshot taken at the last `LOAD` rising edge.
    raw_slots: [bool; HC165_SLOTS],
    /// Frozen snapshot of `raw_slots` as of the last `LOAD` rising edge,
    /// inverted into GPIO *levels* (active-low: pressed = low). Defaults to
    /// all-released (see the module doc's "Reset/pre-LOAD state").
    latched: [bool; HC165_SLOTS],
...
impl Default for Hc165 {
    fn default() -> Self {
        Self {
            raw_slots: [false; HC165_SLOTS],
            latched: [true; HC165_SLOTS], // all-released default; see module doc
            shift_pos: 0,
...
impl Hc165 {
    ...
    /// Sets button slot `idx` (`0..`[`HC165_SLOTS`]) pressed/released. Takes
    /// effect on the *next* `LOAD` rising edge — does not retroactively affect
    /// a snapshot already latched and being shifted out (see module doc).
    ...
    fn on_load_level_changed(&mut self, new_level: bool) {
        if !self.prev_load_level && new_level {
            ...
            for (level, pressed) in self.latched.iter_mut().zip(self.raw_slots.iter()) {
                *level = !*pressed;
            }
            self.shift_pos = 0;
        }
    ...
    fn on_clk_level_changed(&mut self, new_level: bool) {
        if !self.prev_clk_level && new_level && self.prev_load_level {
            ...
            self.shift_pos = (self.shift_pos + 1).min(HC165_SLOTS - 1);
        }
...
mod tests {
    ...
    fn shift_register_presents_set_slots_in_documented_ascending_order() {
        ...
        // slot5(pressed=0) slot6(released=1) slot7(released=1).
    ...
    #[test]
    fn slot_7_is_a_real_settable_button_not_a_constant() {
        // Task 6 change (see the module doc's slot->button table): slot 7
        // carries AUX1 on real hardware, so it must be pressable. Task 4
        // hardcoded it as always-released.
        let mut g = Gpio::new();
        setup_shift_reg_pins(&mut g);
        g.hc165.set_button(7, true);
        pulse_load(&mut g);
        ...
        assert_eq!(
            ...
            "only the 8th presented bit should read pressed"
        );
    }

    #[test]
    fn set_button_ignores_indices_past_the_last_slot() {
        let mut g = Gpio::new();
        setup_shift_reg_pins(&mut g);
        g.hc165.set_button(HC165_SLOTS, true); // must not panic
        g.hc165.set_button(usize::MAX, true);
        pulse_load(&mut g);
        assert!(read_data_bit(&mut g), "nothing should have been pressed");
    }
    ...
    fn shift_position_saturates_at_the_8th_bit_on_extra_clocks() {
        ...
        assert!(read_data_bit(&mut g), "last slot, released");
        for _ in 0..5 {
            clk_rising_edge(&mut g);
            assert!(
                read_data_bit(&mut g),
